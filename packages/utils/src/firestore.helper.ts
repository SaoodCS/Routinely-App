...
export const FIRESTORE_PATHS_AND_FIELDS = {
...
export function getFirestorePathAndField(path: keyof typeof FIRESTORE_PATHS_AND_FIELDS, uid: string): { path: string; field: string } {
   const [pathStr, fieldStr] = FIRESTORE_PATHS_AND_FIELDS[path].split('[');
