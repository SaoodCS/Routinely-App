...
export default function DragAndDropList<TItem extends { id: number | string }>(props: T_DragAndDropListProps<TItem>): JSX.Element {
   ...
   function handlePointerDown(event: PointerEvent<HTMLDivElement>): void {
      ...
      if (button !== 0 || !(target instanceof Element) || dragStateRef.current) return;
      ...
      dragStateRef.current = {
         ...
         itemHeight: height,
         dropThresholdYs: wrapperEls.flatMap((el, index) => {
            if (index === startIndex) return [];
            const { height: wrapperHeight, top: wrapperTop } = el.getBoundingClientRect();
            return [wrapperTop + wrapperHeight / 2 - (index > startIndex ? height : 0)];
         }),
   ...
   function handleDragFrame(): void {
      const dragState = dragStateRef.current;
      if (!dragState) return;
      const { scrollEl, latest, start, pointer, itemHeight, dropThresholdYs } = dragState;
      ...
      const draggedTopY = latest.y - pointer.offsetY + scrollEl.scrollTop - start.scrollTop;
      let low = 0;
      let high = dropThresholdYs.length;
      while (low < high) {
         const mid = (low + high) >> 1;
         if (dropThresholdYs[mid] <= draggedTopY) low = mid + 1;
         else high = mid;
      }
      const nextIndex = low;
      ...
      for (let index = fromIndex; index <= toIndex; index++) {
         ...
         if (!el || index === start.index) continue;
   ...
   function handlePointerEnd(event: PointerEvent<HTMLDivElement>): void {
      ...
      const newOrderedItems = items.toSpliced(start.index, 1).toSpliced(currentIndex, 0, draggedItem);
